    ...
    public class Pager : WebControl, INamingContainer
    {
        ...
        private HyperLink CreateLink(string text, int page, string css = "")
        {
            ...
            if (Component == null || Params == null || Seo == null)
            {
                ...
                var list = HttpContext.Current.Application[SettingsManager.Constants.AppSEOLinkCache + domainCompanyMap[domain].Id] as System.Collections.Generic.List<SEOLinkModel>;
                if (list == null)
                {
            ...
            if (SeoPath) control.NavigateUrl = string.Format("{0}{1}/vit/{2}/{3}/{4}", new URLProvider().BaseUrl, string.IsNullOrEmpty(Component) ? "home" : Component, QueryStringField, page, Params).ToLower();
            else
            {
                control.NavigateUrl = string.Format("{0}{1}vit-{2}-{3}-{4}-{5}", new URLProvider().BaseUrl, Seo, Params, QueryStringField, page, string.IsNullOrEmpty(Component) ? "home" : Component).ToLower();
