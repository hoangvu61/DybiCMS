namespace Web.Asp.Provider.Cache
{
    ...
    public class CacheProvider
    {
        ...
        public static T GetCache<T>(Keys key, Guid companyId, params object[] listParam)
        {
            if (SettingsManager.AppSettings.IsTestEnviroment) return default(T);

            var cacheMap = HttpContext.Current.Application[SettingsManager.Constants.AllMapCache] as Dictionary<Guid, int>;
        ...
        public static void SetCache<T>(T value, Keys key, Guid companyId, params object[] listParam)
        {
            var cacheMap = HttpContext.Current.Application[SettingsManager.Constants.AllMapCache] as Dictionary<Guid, int>;
            if (cacheMap == null)
            {
                cacheMap = new Dictionary<Guid, int>();
        ...
        public static void ClearCache(Guid companyId)
        {
            var cacheMap = HttpContext.Current.Application[SettingsManager.Constants.AllMapCache] as Dictionary<Guid, int>;
            if (cacheMap != null)
            {
