...
<section class="products_section p-5" <%=!string.IsNullOrEmpty(Skin.HeaderBackground) ? "style='background:" + Skin.HeaderBackground + "25'" : ""%>>
    <div class="container font-weight-bold" style="<%=string.IsNullOrEmpty(Skin.HeaderFontColor) ? "" : ";color:" + this.Skin.HeaderFontColor %><%=this.Skin.HeaderFontSize == 0 ? "" : ";font-size:" + this.Skin.HeaderFontSize + "px"%>">
        <%if(HREF.CurrentComponent == "home"){ %>
        <h2 title="<%=Category.Title %>" class="text-center">
            <%=Title %>
        </h2>
        <%} else {%>
            <h1 title="<%=Title %>">
