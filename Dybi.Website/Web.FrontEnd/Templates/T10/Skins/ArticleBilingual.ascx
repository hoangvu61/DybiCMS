...
<h1 title="<%=Data.Details[0].Title %>" class="text-center"><%=Data.Details[0].Title %></h1>
     <%if (!string.IsNullOrEmpty(Data.ImageName))
        { %>
    <div class="text-center">
        <img alt="<%=this.Data.Details[0].Title %>" src="<%=HREF.DomainStore + Data.Image.FullPath %>" style="margin: 0px 20px 20px 20px; max-width:50%"/>
        </div>
    <%} %>
<div class="articlecontent1">
    <%=Data.Details[0].Brief %>
</div>
...
<div class="jumbotron mt-3" id="Translate<%=this.Data.Id %>" style='display:none'>
      <%if (this.Data.Details.Count > 1){ %>
    <h3 title="<%=this.Data.Details[1].Title %>"><%=this.Data.Details[1].Title %></h3>
    <div class="articlecontent2"><%=this.Data.Details[1].Brief %></div>
