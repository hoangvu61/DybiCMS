...
<h1 title="<%=TagName %>">Tag:<strong> <%=TagName %></strong></h1>
<%} %>
<div class="articles" style="margin-top: 30px;">
  <%if(this.Data.Count > 0) { %>
<div id="article<%=this.Data[0].Id %>">
<h2 class="bd-title" title="<%=this.Data[0].Details[0].Title %>"><a href="<%=HREF.LinkComponent("article",this.Data[0].Details[0].Title.ConvertToUnSign(), true, "sart", this.Data[0].Id)%>" title="<%=this.Data[0].Details[0].Title %>"><%=this.Data[0].Details[0].Title %></a></h2>
    <%if (!string.IsNullOrEmpty(Data[0].ImageName))
        { %>
    <a href="<%=HREF.LinkComponent("article",this.Data[0].Details[0].Title.ConvertToUnSign(), true, "sart", this.Data[0].Id)%>" title="<%=this.Data[0].Details[0].Title %>">
        <img alt="<%=this.Data[0].Details[0].Title %>" src="<%=HREF.DomainStore + Data[0].Image.FullPath %>" style="width:250px; float:left; margin: 0px 20px 20px 0px"/>
    </a>
    <%} %>
	<div class="articlecontent1"><%=this.Data[0].Details[0].Content %></div>
<button type="button" class="btn btn-secondary" onclick="$('#Translate<%=this.Data[0].Id %>').toggle(500);"><%=TextTranslate %></button>
  <div class="jumbotron mt-3" id="Translate<%=this.Data[0].Id %>" style="display:none">
      <%if (this.Data[0].Details.Count > 1){ %>
    <h3 title="<%=this.Data[0].Details[1].Title %>"><%=this.Data[0].Details[1].Title %></h3>
    <div class="articlecontent2"><%=this.Data[0].Details[1].Content %></div>
