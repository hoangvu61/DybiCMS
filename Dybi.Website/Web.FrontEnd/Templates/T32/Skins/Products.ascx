...
<div class="product_section layout_padding" style="<%=string.IsNullOrEmpty(this.Skin.BodyBackground) ? "" : ";background-color:" + this.Skin.BodyBackground %>">
    ...
    <div class="product_section_2 layout_padding">
        ...
        <%if(Top > 0 && TotalItems > Top) {%>
            <%if(Top < 9) {%>
                <a href="<%=HREF.LinkComponent("Products", Title.ConvertToUnSign(), true, SettingsManager.Constants.SendCategory, Category.Id, "PromotionOnly", PromotionOnly) %>" style="background:<%=Skin.HeaderBackground%>;color:<%=Skin.HeaderFontColor%>">
                Xem tất cả
                </a>
            <%} else {%>
                <a href="<%=HREF.LinkComponent("Products", Category.Title.ConvertToUnSign(), true, SettingsManager.Constants.SendCategory, Category.Id, "PromotionOnly", true, SettingsManager.Constants.SendPage, 1)%>">Trang đầu</a> 
                ...
                <%} %>
                <a href="<%=HREF.LinkComponent("Products", Category.Title.ConvertToUnSign(), true, SettingsManager.Constants.SendCategory, Category.Id, "PromotionOnly", true, SettingsManager.Constants.SendPage, TotalPages)%>">Trang cuối</a> 
            <%} %>
        <%} %>
        </div>
    </div>
