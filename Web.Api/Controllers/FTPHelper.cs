namespace Web.Api.Controllers
{
    ...
    public class FTPHelper
    {
        ...
        {
            var b = Convert.FromBase64String(file.Base64data);

            string uri = "ftp://" + FTPServerIP + "/" + FTPRootPath + "/" + file.Folder + "-" + file.FileName;
            Connect(uri);

            _reqFTP.KeepAlive = false;
            _reqFTP.Method = WebRequestMethods.Ftp.UploadFile;
            _reqFTP.ContentLength = b.Length;

            using (Stream s = _reqFTP.GetRequestStream())
            {
